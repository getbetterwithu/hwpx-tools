...
{{
  ...
  "cell_fills": [
    {{"tid": 123, "text": "빈 셀에 채울 텍스트"}}
  ]
}}
```

- `replacements`: 기존 텍스트를 다른 텍스트로 바꿀 때 사용 (find-and-replace).
- `cell_fills`: **표의 빈 셀에 새 텍스트를 채울 때**만 사용. 본문에서 `⟦CELL:N⟧` 형식의 마커가 보이면 그 셀이 비어 있다는 뜻이고, N이 그 셀의 tid 정수. 채우려면 `{{"tid": N, "text": "..."}}`로 답해라.
...
8. **표의 빈 셀**: 본문에 `⟦CELL:N⟧` 형식으로 표시됩니다. 이런 셀을 채우려면 `replacements`가 아니라 `cell_fills`에 `{{"tid": N, "text": "..."}}`를 넣어라. N은 정수. 절대 마커 문자열 자체를 `old`로 쓰지 말 것 — `cell_fills`로만 처리됨.
9. 표는 본문에서 markdown 표 형식으로 보입니다(`| 헤더 | ... |`). 표 안의 빈 셀이 `⟦CELL:N⟧`로 표시되므로, 표 구조(어느 행/열인지)를 보고 적절한 값을 골라 채워라.
